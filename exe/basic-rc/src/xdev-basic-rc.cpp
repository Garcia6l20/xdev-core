...
public:
    ...
    ~scoped_file() { if (_file) fclose(_file); }
...
private:
    ...
    void open(std::string_view mode) {
        ...
        if (not _file) {
            throw std::runtime_error("unable to open " + _path.string());
        }
    }
...
void XBasicResourceCompiler::processFile(const fs::path& path, const string& root)
{
...

    {
        ...
        fmt::print(*file, "// key: {}\n"
                          "const uint8_t {}_data[] = {{", basename, variable_name);
        ...
        fmt::print(*file, "const size_t {v}_len = sizeof({v}_data);\n"
...
void XBasicResourceCompiler::createResourceFiles() {
...

    {
        ...
        fmt::print(*cpp, "    return resources;\n"
                         "}}\n\n"
                         "const XResources::ptr {}Resources = _load_resources();", base_name);
    }
}
