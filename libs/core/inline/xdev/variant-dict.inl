...
namespace xdev::variant {
  ...
  Dict<StringPolicy>::Dict()
      : _value() {}

  template <typename StringPolicy>
  Dict<StringPolicy>::Dict(const DictInitList<StringPolicy> &value)
      : _value(value) {}

  template <typename StringPolicy>
  Dict<StringPolicy>::Dict(Dict &&other)
      : _value(std::move(other._value)) {}

  template <typename StringPolicy>
  Dict<StringPolicy> &Dict<StringPolicy>::operator=(Dict &&other) {
    _value = std::move(other._value);
    return *this;
  }

  template <typename StringPolicy>
  Dict<StringPolicy>::Dict(const Dict &other)
      : _value(other._value) {}

  template <typename StringPolicy>
  Dict<StringPolicy> &Dict<StringPolicy>::operator=(const Dict &other) {
    _value = other._value;
    return *this;
  }
  ...
  template <typename StringPolicy>
  decltype(auto) Dict<StringPolicy>::begin() {
    return _value.begin();
  }

  template <typename StringPolicy>
  decltype(auto) Dict<StringPolicy>::end() {
    return _value.end();
  }

  template <typename StringPolicy>
  decltype(auto) Dict<StringPolicy>::begin() const {
    return _value.cbegin();
  }

  template <typename StringPolicy>
  decltype(auto) Dict<StringPolicy>::end() const {
    return _value.cend();
  }

  template <typename StringPolicy>
  size_t Dict<StringPolicy>::size() const {
    return _value.size();
  }
  ...
  std::string Dict<StringPolicy>::toString() const {
    ...
    res += tools::join(*this, ", ", [](auto &&item) {
      ...
      if (item.second.template is<std::string>()) tmp += std::string("\"") + item.second.toString() + "\"";
      else
        tmp += item.second.toString();
      return tmp;
    });
    res += "}";
    return res;
  ...
  inline Dict<StringPolicy> &Dict<StringPolicy>::update(Dict &&other) {
    for (auto &&[k, v] : other._value) { _value.emplace(std::move(k), std::move(v)); }
    return *this;
  }
  ...
  Variant<StringPolicy> &Dict<StringPolicy>::at(Value<StringPolicy> &&key, RestT &&... rest) {
    try {
      ...
      if (!key.template is<std::string>()) throw e;
      // handle dot notation
  ...
  template <typename StringPolicy>
  Variant<StringPolicy> &Dict<StringPolicy>::dotAt(std::string_view key) {
    return const_cast<Variant<StringPolicy> &>(static_cast<const Dict<StringPolicy> &>(*this).dotAt(key));
  }
