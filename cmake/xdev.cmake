...
function(xdev_moc_target target)
    ...
    get_target_property(inlcude_dirs ${target} INTERFACE_INCLUDE_DIRECTORIES)
    list(FILTER header_files INCLUDE REGEX ".*\.h(pp)?$")
    set(stamp ${CMAKE_CURRENT_BINARY_DIR}/${target}-moc-stamp)
    set(generated_files "")
    set(data "")
    foreach(file ${header_files})
        get_source_file_property(_is_generated ${file} GENERATED)
        if(${_is_generated})
          continue()
        endif()
        get_filename_component(_filename ${file} NAME_WE)
        get_filename_component(_dir ${file} DIRECTORY)
        list(APPEND generated_files ${CMAKE_CURRENT_BINARY_DIR}/${_filename}.xdev.hpp)
        list(APPEND generated_files ${CMAKE_CURRENT_BINARY_DIR}/${_filename}.xdev.cpp)
        list(APPEND data "{\"full_path\": \"${CMAKE_CURRENT_BINARY_DIR}/${file}\", \"path\": \"${file}\", \"name\": \"${_filename}\", \"directory\": \"${_dir}\"}")
    endforeach()
    ...
    list(JOIN data ", " data)
    list(JOIN inlcude_dirs "\", \"" inlcude_dirs)
    set(inlcude_dirs "\"${inlcude_dirs}\"")
    set(project_data "{\"project\": \"${PROJECT_NAME}\", \"target\": \"${target}\", \"source_dir\": \"${CMAKE_CURRENT_SOURCE_DIR}\", \"bin_dir\": \"${CMAKE_CURRENT_BINARY_DIR}\", \"include_dirs\": [${inlcude_dirs}], \"headers\": [${data}]}")
    message(STATUS "project_data: ${project_data}")
    add_custom_command(
        OUTPUT ${generated_files}
        DEPENDS ${_deps}
        COMMAND ${MOC_EXE_OR_TARGET} ARGS "${project_data}"
        VERBATIM
    )
